...
const parsePrometheusText = (text: string): Record<string, number> => {
  ...
  text.split('\n').forEach((line) => {
    ...
    const baseName = metricPart.includes('{')
...
const pickBytesFromPrometheus = (
  p: Record<string, number>
): { rx: number | null; tx: number | null } => {
  const tryKeys = (keys: string[]): number | null => {
    const key = keys.find((item) => Number.isFinite(p[item]));
    return key ? p[key] : null;
  };
  let rx = tryKeys([
    'nanomq_bytes_received',
    'nanomq_recv_bytes',
    'nanomq_network_bytes_received',
  ]);
  let tx = tryKeys([
    'nanomq_bytes_sent',
    'nanomq_sent_bytes',
    'nanomq_network_bytes_sent',
  ]);
  if (rx === null && tx === null) {
    Object.entries(p).forEach(([k, v]) => {
      if (!Number.isFinite(v)) return;
      const kl = k.toLowerCase();
      if (!kl.includes('byte')) return;
      if (
        kl.includes('recv') ||
        kl.includes('receiv') ||
        kl.includes('inbound')
      ) {
        rx = rx === null ? v : Math.max(rx, v);
      } else if (
        kl.includes('sent') ||
        kl.includes('transmit') ||
        kl.includes('outbound')
      ) {
...
function baseMetricsInfoFromRaw(raw: unknown): MetricsInfo {
  ...
  const memoryTotal =
    memTotalRaw > 0
      ? Math.max(memTotalRaw, memUsage, 1)
      : Math.max(memUsage, 1);
  const connections = toSafeNumber(
    data.connections_count !== undefined
      ? data.connections_count
      : data.connections,
    0
  );
  const subs = toSafeNumber(
    data.subscriptions_count !== undefined
...
function mergePrometheusNodesAndSubs(
  ...
  if (promText.length > 0) {
    ...
    if (Number.isFinite(p.nanomq_cpu_usage)) {
      merged.cpu_usage = p.nanomq_cpu_usage;
    }
    if (Number.isFinite(p.nanomq_memory_usage)) {
      merged.memory_usage = p.nanomq_memory_usage;
    }
    if (
      Number.isFinite(p.nanomq_memory_usage_max) &&
      p.nanomq_memory_usage_max > 0
    ) {
      merged.memory_total = Math.max(
        p.nanomq_memory_usage_max,
        merged.memory_usage || 0,
        1
      );
    }
    if (Number.isFinite(p.nanomq_connections_count)) {
      merged.connections_count = p.nanomq_connections_count;
    }
    if (Number.isFinite(p.nanomq_connections_max)) {
      merged.connections_max = p.nanomq_connections_max;
    }
    if (Number.isFinite(p.nanomq_subscribers_count)) {
      merged.subscriptions_count = Math.max(
        p.nanomq_subscribers_count,
        subscriptionCount
      );
    } else if (subscriptionCount > 0) {
      merged.subscriptions_count = subscriptionCount;
    }
    if (Number.isFinite(p.nanomq_subscribers_max)) {
      merged.subscriptions_max = p.nanomq_subscribers_max;
    }
    if (Number.isFinite(p.nanomq_messages_received)) {
      merged.messages_received = p.nanomq_messages_received;
    }
    if (Number.isFinite(p.nanomq_messages_sent)) {
      merged.messages_sent = p.nanomq_messages_sent;
    }
    if (Number.isFinite(p.nanomq_messages_dropped)) {
      merged.messages_dropped = p.nanomq_messages_dropped;
    }
    if (Number.isFinite(p.nanomq_messages_retained)) {
      merged.messages_retained = p.nanomq_messages_retained;
    }
  ...
  if (nodes && nodes.length > 0) {
    ...
    if (
      conns !== null &&
      (merged.connections_count === 0 || merged.connections_count === undefined)
    ) {
  ...
  if (
    !Number.isFinite(merged.memory_total || NaN) ||
    (merged.memory_total || 0) <= 0
  ) {
...
export interface MergedMetricsInput {
  metricsRaw: unknown;
  promText: string;
  nodes: NodeInfo[];
  subscriptionCount: number;
